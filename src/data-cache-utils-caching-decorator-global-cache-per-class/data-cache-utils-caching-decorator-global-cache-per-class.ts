 ...
 * of a cached values
 */
export const dataCachingUtilsCachingDecoratorGlobalCachePerClass = <
  T,
  V,
  I extends object
>(
  ...
  type TMapKey = TGetKeyByMethodArgument extends (value: T) => any
    ? ReturnType<Exclude<TGetKeyByMethodArgument, undefined>>
    : T;
  /**
   *
   *
  ...
  return (
    ...
    const methodOrigin: IDataCachingDecoratorDecoratedFunction<T, V> =
      descriptor.value as IDataCachingDecoratorDecoratedFunction<T, V>;
    // key - rating
    ...
    const removeValuesWithWorstRatingFromCacheAndUpdateMinimalRating = debounce(
      (): void => {
        ...
        keysSortedByRating.sort(
          (keyWithRatingFirst, keyWithRatingSecond) =>
            keyWithRatingSecond.rating - keyWithRatingFirst.rating
        );
    ...
    async function cachingWrapper(this: I, parameter: T): Promise<V> {
      const key = getKeyByMethodArgument
        ? (getKeyByMethodArgument(parameter) as TMapKey)
        : parameter;
      ...
      const whetherKeyRatingIsGreaterThanMinimal =
        keyRatingUpdated > minimalRatingOfValueInCache;
