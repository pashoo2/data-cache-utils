...
describe('Caching decorator', () => {
  ...
  let valueGetter: jest.MockedFunction<
  ...
  beforeEach(() => {
    ...
    valueGetter = jest.fn(async (key: string): Promise<string | undefined> => {
      return map.get(key);
    });
  });
  ...
  it('Should return undefined for a key which is not exists', async () => {
    ...
    await expect(testClassImplementation.getValue('unknown_key')).resolves.toBe(
      undefined
    );
  });
  ...
  it('Should use the same cache for all instances of class', async () => {
    ...
    expect(valueGetter).not.toBeCalled();
    await expect(testClassImplementation.getValue(testKey)).resolves.toBe(
      testValue
    );
    ...
    await expect(testClassImplementationSecond.getValue(testKey)).resolves.toBe(
      testValue
    );
    expect(valueGetter).toBeCalledTimes(1);

    const testClassImplementationThird = new TestClass();
    await expect(testClassImplementationThird.getValue(testKey)).resolves.toBe(
      testValue
    );
    expect(valueGetter).toBeCalledTimes(1);
  });
  ...
  describe('Run each test with its own cache', () => {
    ...
    it('Should not call getter function twice also for the second value', async () => {
      ...
      expect(valueGetter).toBeCalledTimes(1);
      await expect(
        testClassImplementation.getValue(testKeySecond)
      ).resolves.toBe(testValueSecond);
      expect(valueGetter).toBeCalledTimes(2);
      await expect(
        testClassImplementation.getValue(testKeySecond)
      ).resolves.toBe(testValueSecond);
    ...
    it('Should call getter function for a value whose rating less than the existing values in cache', async () => {
      ...
      await expect(
        testClassImplementation.getValue(testKeyThird)
      ).resolves.toBe(testValueThird);
